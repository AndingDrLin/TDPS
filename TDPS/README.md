...
TDPS 是 Team 15 智能巡线小车项目，目标 MCU 为 **STM32F407VET6**。当前主固件统一放在 `firmware/`，仿真、测试、文档和报告已整理到同一项目目录：

- 巡线控制：8 路灰度传感器、标定、归一化、滤波、PID、直道高速、弯前减速、抗干扰、丢线恢复和岔路识别
- 雷达避障：HLK-LD2410S 串口帧解析，输出 `CLEAR/WARN/BLOCK`，阻塞时进入开环绕障并重新寻线
- 无线通信：EWM22A-900BWL22S LoRa 异步队列、超时重试、可选 ACK

## 任务要求

- 巡线：从起点沿 3 cm 黑线稳定行驶，支持直道高速、弯前减速、箭头/宽黑干扰保护、岔路识别和可信方向丢线恢复，恢复超时必须停车。
...
Simulator 代码已整理到 `simulator/`。如果脚本仍假设旧路径 `TDPS-Simulator/`，先阅读 `agent_docs/03_agent_working_notes.md` 再调整或从兼容路径运行。

```bash
bash simulator/scripts/line_follow_cli.sh quick
bash simulator/scripts/line_follow_cli.sh full-course \
  simulator/sim_tests/line_follow_v1/config/scenarios_full_course.csv \
  90 0.01 0.12 \
  simulator/artifacts/line_follow_v1/reports/single_run/full_course_stress.json \
  20260319 stress
bash simulator/scripts/run_line_follow_stability.sh
```

Simulator runner 用于编译统一 `firmware/` 树，固定 8 路传感器，并覆盖 normal/stress 扰动、完整赛道、雷达障碍、检查点和无线队列诊断。

## 关键行为

- 巡线输入固定为 8 路，支持 ADC 模拟量、8-LP 数字 GPIO 和 8-LP UART 协议模式；当前默认 debug profile 使用 UART 模式和极性翻转。
- `main.c` 默认调用 `LF_Config_ApplyDebugProfile()`，使用低速保守巡线参数；只有明确切换时才使用 `LF_Config_ApplyCompetitionProfile()`。
...
- 报文格式：`TEAM=<id>,NAME=<name>,CP=<checkpoint>,TIME=<MM:SS>\n`。

## 文档

- agent 快速上下文：`agent_docs/`
- 固件入口：`firmware/`
- 仿真入口：`simulator/`
- 板级/独立测试：`tests/`
...
- 测试说明：`docs/testing/`
- 硬件资料索引：`docs_reference/README.md`
- 报告脚本：`reports/`
