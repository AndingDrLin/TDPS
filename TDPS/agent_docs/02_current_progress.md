...
- track profile 默认启用岔路识别，但箭头形干扰保护也会抑制疑似宽黑跳变；实车需要分别验证箭头路口和真实岔路。
...
3. 在实车上先架空确认 `Profile track` 打印和电机方向，再依次测试直道高速、箭头干扰、U 型弯和岔路识别。
4. 明确左右分叉路线的障碍检测方案，避免只用前向雷达做无法区分左右侧的决策。
5. 将每次上板测试的日期、场地、电池电压、track 参数和现象写入实验记录。
