...
/*
 * UpdatePD 的参数覆盖版本：用于分段控制动态切换参数集。
 * 与 UpdatePD 逻辑完全一致，但不再从 g_lf_config 读取 kp/kd/kff/max_correction，
 * 而是使用调用方传入的值。derivative_filter_alpha 和 max_output_delta_per_tick
 * 仍从 g_lf_config 读取（这两个参数通常无需分段切换）。
 */
